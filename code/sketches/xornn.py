...
def generate_xor_dataset(N):
    ...
    for r in ranges:
        for i in range(nq):
            p = np.array([random.uniform(r.x[0], r.x[1]), random.uniform(r.y[0], r.y[1])])
            X.append(p)
            y.append(r.label)
